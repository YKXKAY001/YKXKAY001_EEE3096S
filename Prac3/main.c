...
void EXTI0_1_IRQHandler(void)
{
	...
	// Check if at least 50ms have passed since last call, otherwise ignore function call.
	if (currentTick - prevTick > 50){
		if (toggle == 0){
			delay_t = delay_t / 2;
			toggle = 1;
		}
...
uint32_t pollADC(void){
  // TODO: Complete function body to get ADC val
	...
	HAL_ADC_PollForConversion(&hadc, 100); // CPU waits while ADC converts input

	val = HAL_ADC_GetValue(&hadc);		   // Get ADC output value
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	...
	// 11.7213675 is 47999 (arr) / 4095 (adc max)
	// Not a versatile solution but it prevents the repeated arr/adc_max calculation
	// ADC_Val * arr/adc_max = ccr

	uint32_t val = (uint32_t) (adc_val * (11.7213));
	return val;
}
